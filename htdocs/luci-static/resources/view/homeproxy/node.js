 ...
 * Modified for Airport & Region Group Management
 */
...
return view.extend({
    ...
    render(data) {
        ...
        /* 🚀 核心改造 1：读取新的 airport 模型，废弃旧的 subscription_url 🚀 */
        let subinfo = [];
        uci.sections(data[0], 'subscription_airport', (sec) => {
            if (sec.url) {
                subinfo.push({ 
                    'hash': sec['.name'], // 使用 UCI 内部 ID 作为唯一标识
        ...
        so.textvalue = function(section_id) {
            return E('button', {
                ...
                'click': (ev) => {
                    ...
                    return this.map.save(null, true).then(() => {
                        return fs.exec_direct('ucode', ['/etc/homeproxy/scripts/update_subscriptions.uc', section_id]);
                    }).then((res) => {
        ...
        o.onclick = function() {
            return this.map.save(null, true).then(() => {
                ...
                return fs.exec_direct('ucode', ['/etc/homeproxy/scripts/update_subscriptions.uc']).then((res) => {
                    clearInterval(log_timer);
                    let match = res.match(/FETCH_SUCCESS:(\d+)/);
        ...
        ss.filter = function(section_id) {
            let airport_id = uci.get(data[0], section_id, 'airport_id'); // 新主键
            let grouphash = uci.get(data[0], section_id, 'grouphash');   // 旧主键
            if (airport_id || grouphash) return false;
            return true;
        }

        /* Import subscription links start (保留手工导入功能) */
