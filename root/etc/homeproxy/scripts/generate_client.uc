...
if (!isEmpty(main_node)) {
    ...
    if (routing_mode === 'bypass_mainland_china') {
        ...
        push(config.dns.rules, { type: 'logical', mode: 'and', rules: [{ rule_set: 'geosite-noncn', invert: true }, { rule_set: 'geoip-cn' }], action: 'route', server: 'china-dns' });
        
        let bypass_evaluate_rules = [
            { action: 'evaluate', server: 'main-dns' },
            { action: 'route', match_response: true, rule_set: 'geoip-cn', server: 'china-dns' },
            { action: 'respond' }
        ];
        config.dns.rules = [...config.dns.rules, ...bypass_evaluate_rules];
    }
